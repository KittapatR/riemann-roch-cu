...
$\mathbb{K}$-linear map $D_\gamma: \mathcal{O}_{M,p} \to \mathbb{K}$, $f \mapsto (f \circ c)'(0)$ ($[(U,f)]$) for $I \mathring\supseteq c^{-1}(U) \to \mathbb{K}$ with the property that
$\forall f,g \in \mathcal{O}_{M,p}: D_\gamma(f\cdot g) = ((f\circ c)(g \circ c))'(0)$ which holds the product rule of derivative.

---

**Def** Let $R$ be a ring, $A$ be an $R$-algebra (commutative with unit), $M$ be a model.
...
2. Let $A$ and $B$ be localring with maximal ideals $\mathfrak{m}_A$ and $\mathfrak{m}_B$ respectively. A *local ring homomorphism* ($A \to B$) is a ring homomorphism $\varphi: A \to B$ such that $\varphi(\mathfrak{m}_A) \subseteq \mathfrak{m}_B$, or equivalently (!) $\varphi^{-1}(\mathfrak{m}_B) = \mathfrak{m}_A$.
...
- Let $(M, \mathcal{O}_M)$ be a premanifold, $p\in M$. (Claim that $\mathcal{O}_{M,p}$ is local.)
	...
	that $\mathcal{m}_{M,p} := \ker(\text{ev}_p) = \{f \in \mathcal{O}_{M,p}\,|\,f(p) = 0\}$ is a maximal ideal of $\mathcal{O}_{M,p}$! (Apply the *first isomorphism theorem*, $\mathcal{O}_{M,p}/\mathfrak{m}_{M,p} \cong \mathbb{K}$ since $\text{ev}_p$ is a surjective.)
...
*Remark*. The category of locally $R$-ringed spaces is a subset of the one of $R$-ringed spaces but not a full one!

---

*Exercise* Let $k$ be a field. Show that a concrete $k$-space $(X, \mathcal{O}_X)$ is a locally $k$-ringed space if $\forall U \mathring\subseteq X, \forall f \in \mathcal{O}_X(U)$: $f$ nowhere also on $U$: $\frac1f \in \mathcal{O}_X(U)$.
...
In particular, every such a morphism of locally $k$-ringed spaces.
...
\begin{document}
\begin{tikzcd}
f \arrow[rrr, maps to]\arrow[dddd,maps to]&[-2em]&&[-2em]\varphi_a^\flat(f)\arrow[dddd,maps to]\\[-4ex]
& \mathcal{O}_Y(U) \arrow[r,"\varphi_U^\flat"]\arrow[d] & \mathcal{O}_X(\varphi^{-1}U) \arrow[d] & \\
& \mathcal{O}_{Y, \varphi(a)} \arrow[r, "\varphi_U^\sharp"]\arrow[d, "\text{ev}_{\varphi(a)}"] & \mathcal{O}_{X,a} \arrow[d, "\text{ev}_{\varphi(a)}"] &\\
& k \arrow[r, "?"] & k &\\[-4ex]
f(\varphi(a) \arrow[rrr, maps to] &&& \varphi_a^\flat(f)(a)
\end{tikzcd}
\end{document}
```
...
*Def* Let $M$ be a $C^\infty$/$C^\omega$/complex-premanifold, $p \in M$, the algebraic tangent space of $M$ at $p$ $$T^\text{alg}_pM  =\text{Der}_\mathbb{K}(\mathcal{O}_{M,p}, \mathbb{K})$$

Let $F: M \to N$ be a morphism of (...)-premanifolds, $a\in M$, then the tangent map is defined by
$T^\text{alg}_p(F): T_p^{alg}(M) \to T_p^{alg}(N)$, that $\partial \mapsto \partial \circ F^{\sharp}_a$ (Check that it is a $\mathbb{K}$-derivation.)
...
\begin{document}
\begin{tikzcd}
\gamma\arrow[ddd,maps to]\arrow[rrr,maps to]&[-2em]&&[-5em] F_*\gamma\arrow[ddd,maps to]\\[-4ex]
&T^\mathrm{geo}_pM \arrow[d] \arrow[r, "F_*"] & T^\mathrm{geo}_pN \arrow[d]& \\
&T^\mathrm{alg}_pM \arrow[r, "F_a^\sharp"] & T^\mathrm{alg}_pN&\\[-4ex]
D_\gamma\arrow[rrr,maps to]&&&D_\gamma \circ F_a^\sharp \overset{?}{=} D_{F_*\gamma}
\end{tikzcd}
\end{document}
```
...
$k$ can be interpreted as $A$-model via $A/\mathfrak{m}$. ($\mathfrak{m}^2 = \text{ideal of }A\text{ generated by }f\circ g: f,g \in \mathfrak{m}$.)
...
- Claim. $a \in \mathbb{R}^n$: $\mathcal{C}^\infty_{\mathbb{R}^n, a}, \mathcal{C}^\omega_{\mathbb{R}^n, a}$ satisfy the tangent space condition, also with $a\in \mathcal{C}^n$, so is $\mathcal{O}^h_{\mathbb{C}^n,a}$ (exercise: verify this. Arapura, 2.5.18)
